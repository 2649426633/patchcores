...
EXPERIMENTS = [
    ...
    ("l2", ("layer2",), Path("products/screw/models/patchcore_320_l2")),
    ("l3", ("layer3",), Path("products/screw/models/patchcore_320_l3")),
]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Ablate PatchCore feature layers at fixed 320x320 input resolution. "
            "Compare layer2-only, layer3-only, and the current layer2+layer3 baseline."
        )
    )
    ...
    parser.add_argument("--work-dir", default="outputs/screw/single_layer_ablation_320")
...
def summarize(name: str, layers: tuple[str, ...], model_dir: Path, csv_path: Path) -> dict:
    ...
    by_type = {
        defect_type: metrics([r for r in rows if r.get("defect_type") == defect_type])
...
def main():
    ...
    if not summaries:
        raise RuntimeError("No successful single-layer experiments.")

    summary_path = args.work_dir / "single_layer_summary.csv"
